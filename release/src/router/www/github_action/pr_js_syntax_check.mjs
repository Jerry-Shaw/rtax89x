...
async function main() {
    ...
    if (owner !== "" && repo !== "" && pull_number !== "") {
        ...
        await octokit.request('GET /repos/{owner}/{repo}/pulls/{pull_number}/files', {
            ...
            let filtered = fileList.filter(path => path.includes('release/src/router/www'));
            prFiles = filtered.map(path => path.replace('release/src/router/www', ''));
        });

        console.log("> Pull Request Files:", prFiles);

        const eslint = new ESLint({
            overrideConfigFile: 'pr_js_eslint.config.mjs'
        });
        ...
        for (const dist of dists) {
            ...
            for (const file of prFiles) {
                if ((path.extname(file) === '.asp' || path.extname(file) === '.html' || path.extname(file) === '.htm') && !ignoreFiles.includes(file) && !ignoreFolders.some(ignoreFolder => file.includes(ignoreFolder))) {
                    process.stdout.write(`[${dist}] File: ${file}`);
                    ...
                    await $('script').each((i, elem) => {
                        ...
                        if (src !== file) {
                            ...
                            try {
                                ...
                                console.log('\x1b[31m%s\x1b[0m', `[${dist}] File: ${file} -> Fail!`)
                        ...
                        if (!(result.errorCount === 0 && result.warningCount === 0)) {
                            result.messages.forEach(message => {
                                if (!message.message.includes('Unused eslint-disable directive')) {
                                    ...
                                    readline.clearLine(process.stdout, 0);
                                    console.log('\x1b[31m%s\x1b[0m', `[${dist}] File: ${file} -> Fail!`)
                                    console.error(`Lang: ${dist}, File: ${file}, Error File: ${scriptObject.src}, Line: ${realErrorLineNumber}\n\tJavaScript syntax error: ${message.message} \n\tCode: ${selectedContent}`);
                                    errors.push({
                                        dist,
                            ...
                            readline.cursorTo(process.stdout, 0);
                            readline.clearLine(process.stdout, 0);
                            console.log('\x1b[32m%s\x1b[0m', `[${dist}] File: ${file} -> Pass!`);
                        }
                    }).catch(error => {
                        ...
                        if (!(result.errorCount === 0 && result.warningCount === 0)) {
                            result.messages.forEach(message => {
                                if (!message.message.includes('Unused eslint-disable directive')) {
                                    ...
                                    readline.clearLine(process.stdout, 0);
                                    console.log('\x1b[31m%s\x1b[0m', `[${dist}] File: ${file} -> Fail!`)
                                    console.error(`Lang: ${dist}, File: ${file}, Line: ${message.line}\n\tJavaScript syntax error: ${message.message} \n\tCode: ${selectedContent}`);
                                    errors.push({
                                        dist,
                                        file: file,
                                        line: message.line,
