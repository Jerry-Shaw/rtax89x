...
function initial(){
	...
	if(smart_connect_flag_t == '0' || smart_connect_flag_t == ''){
		...
		if(odmpid === 'GT6'){
			band0_channel = '<% nvram_get("wl2_chanspec"); %>';
			band1_channel = '<% nvram_get("wl0_chanspec"); %>';
			band2_channel = '<% nvram_get("wl1_chanspec"); %>';
		}
	...
	if(unii4Support ){
		if(wl_unit == '1' && !band5g2_support && document.form.wl_channel.value == '0'){
			...
			if(wl_unit == '0' && band6g_support && document.form.wl_channel.value == '0'){
				document.querySelector('#acs_unii4_field').style.display = '';
			}
			else if(wl_unit == '1' && document.form.wl_channel.value == '0'){
				document.querySelector('#acs_unii4_field').style.display = '';
...
function genBWTable(_unit){
	...
	if(document.form.wl_nmode_x.value == 2){
		bws = [1];
		bwsDesc = ["20 MHz"];
		if(tmo_support && _unit == 2){		// for 2.4G B/G Mixed
	...
	if(validForm()){
		...
		if(document.form.smart_connect_x.value != '0'){
			if(document.form.smart_connect_t.value == '1'){
				...
				// document.form.wl0_bw.value = document.form.band1_bw.value;
				// document.form.wl0_chanspec.value = document.form.band1_channel.value;
				// if ($('#band1_acsDFS').is(':visible')) {
				// 	if ($('#band1_acsDFS_checkbox').is(':checked')) {
				// 		document.form.acs_dfs.value = 1;
				// 	}
				...
				// if ($('#band1_160').is(':checked')) {
				// 	document.form.wl0_bw_160.value = 1;
				// }
				// else {
				// 	document.form.wl0_bw_160.value = 0;
				// }
			...
			else if(wl_unit == '1' && band5g2_support){
			...
			else if(wl_unit == '0' && band6g_support){
				if(document.form.acs_dfs_checkbox.checked){
					document.form.acs_dfs.value = "1";
					document.form.acs_band3.value = "1";
				}
	...
	if(val == 0){
		...
		if(val == '1'){
			...
			if(wl_unit == '2'){
				...
				document.form.wl0_chanspec.disabled = false;
				document.form.wl1_chanspec.disabled = false;
				document.form.wl0_bw_160.disabled = false;
				document.form.wl1_bw_160.disabled = false;
				$("#band_separate").show();
...
<input type="hidden" name="wl0_bw_160" value='<% nvram_get("wl0_bw_160"); %>'>
				...
				<tr id="wl_unit_field">
					...
					<td>
						<select name="wl_unit" class="input_option" onChange="_change_wl_unit(this.value);">
							<option class="content_input_fd" value="2" <% nvram_match("wl_unit", "2","selected"); %>>2.4 GHz</option>
							<option class="content_input_fd" value="0" <% nvram_match("wl_unit", "0","selected"); %>>5 GHz</option>
							<option class="content_input_fd" value="1" <% nvram_match("wl_unit", "1","selected"); %>>5 GHz-2</option>
						</select>			
					</td>
		  	</tr>
				...
				<tr>
					...
					<td>
						...
						<span id="band1_160_field" style="display:none"><input id="band1_160" type="checkbox" onClick="separateEnable_160MHz(this);" <%
							 nvram_match("wl0_bw_160", "1" , "checked" ); %>>
