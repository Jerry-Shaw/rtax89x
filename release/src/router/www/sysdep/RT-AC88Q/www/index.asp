...
function update_statusframe() {
	window.open( "device-map/router.asp", "statusframe", '' );
...
function initial(){
	...
	//Build Router Wireless and Guestnetwork icon
	var wirelessHtml = "";
	var wl_vifnames = '<% nvram_get("wl0_vifnames"); %>';
	var gn_conut = wl_vifnames.split(" ").length;
	if(multissid_support && (sw_mode == 1 || sw_mode == 3)) {
		if(gn_conut > 3)
			wirelessHtml += "<div style='float:left;margin-top:37px;margin-left:10px;'>";
		else
			wirelessHtml += "<div style='float:left;margin-top:10px;margin-left:10px;'>";
	}
	else {
		wirelessHtml += "<div style='margin-top:6px;'>";
	}
	wirelessHtml += "<a href='device-map/router.asp' target='statusframe'><div id='iconRouter' class='iconRouter' onclick='clickEvent(this);'></div></a>";
	wirelessHtml += "</div>";

	if(sw_mode == 1 || sw_mode == 3) {
		for(var i = 0; i < gn_conut; i += 1) {
			if(gn_conut > 3)
				wirelessHtml += "<div id='iconGuestnetwork_" + (i + 1) + "' class='iconGuestnetwork' style='margin-left:6px;margin-top:10px;' onclick='clickEvent(this);update_statusframe();'>Guest " + (i + 1) + "</div>";
			else
				wirelessHtml += "<div id='iconGuestnetwork_" + (i + 1) + "' class='iconGuestnetwork' style='margin-left:6px;margin-top:25px;'onclick='clickEvent(this);update_statusframe();'>Guest " + (i + 1) + "</div>";
		}
	}

	document.getElementById("routerWirelessState").innerHTML = wirelessHtml;


	wirelessHtml = "";
	wirelessHtml += "<div style='clear:both;'></div>";
	wirelessHtml += "<div style='background-color:#2D393D;margin:6px 10px;border-radius:10px;text-align:left;border-style:solid;border-color:rgb(82, 92, 95);border-width: 1px;'>";
	if(sw_mode == "4"){
		wirelessHtml += "<div style='width:100%;text-align:center;height:30px;line-height:30px;'><#OP_MB_item#></div>";
	}
	else{
		for(var i = 0; i < wl_nband_title.length; i += 1) {
			wirelessHtml += "<div style='height:30px;line-height:30px;'>";
			wirelessHtml += "<div style='color:#00c6ff;padding-left:10px;height:30px;width:30%;float:left;'>" + wl_nband_title[i] + " SSID</div>";
			wirelessHtml += "<div id='wl_band_" + i + "' style='width:66%;float:right;height:30px;'></div>";
			wirelessHtml += "</div>";
		}
	}
	wirelessHtml += "</div>";
...
function show_ddns_status(){
	...
	if( ddns_enable == '0')
        ...
        if(ddns_enable == '1'){
		    ...
		    if(!fail && le_enable == "1" && le_state == "1")
	...
	if(obj.id.indexOf("Internet") > 0){
		...
		if(sw_mode != "4"){
			...
			if(wl_info.band5g_support)
				document.getElementById("wl_band_1").innerHTML = decodeURIComponent('<% nvram_char_to_ascii("", "wl1_ssid"); %>');
			if(wl_info.band5g_2_support)
				document.getElementById("wl_band_2").innerHTML = decodeURIComponent('<% nvram_char_to_ascii("", "wl2_ssid"); %>');
		}
	}
	...
	if(document.getElementById("edit_client_block").style.display != "none" && document.getElementById('client_name').value == clientName){
		...
		if(sw_mode == "1" && !clientObj.amesh_isRe) {
			...
			if(pm_support) {
				...
				document.getElementById('adv_time_scheduling').style.display = "none";
			}
		}
		else {
	...
	<td valign="top" width="17">&nbsp;</td>
		...
		<div id="NM_table_div">
			...
			<table id="_NM_table" border="0" cellpadding="0" cellspacing="0" height="720" style="opacity:.95;" >
				...
				<tr>
					<td colspan="3" align="center" bgcolor="#444f53" class="NM_radius" style="vertical-align:top;height:auto;">
