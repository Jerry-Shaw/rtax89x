...
const char sdservers[] = "/tmp/resolv.smartdns";
#else
...
int build_temp_rootfs(const char *newroot)
{
	...
	const char *bin = "ash busybox cat cp dd df echo grep iwpriv kill ls ps mkdir mount nvram ping sh tar umount uname rm chmod mv klogd syslogd sed sleep gzip gunzip"
    ...
    defined(RTCONFIG_WEBDAV)
			     ...
			     " libpam.so* openvpn-plugin-auth-pam.so*"
#endif
	...
	const char *kmod = "find /lib/modules -name '*.ko'|"
		...
		"ip6t_REJECT\\|ip6t_ROUTE\\|ip6t_LOG\\|"
#endif
#if defined(RTCONFIG_OPENVPN)
		"tun\\|"
#endif
		"nf_conntrack\\|nf_nat\\|nfnetlink\\|xt_\\|"
...
#endif
#endif
	__cp("R", "/usr/lib/xtables", "*.so", newroot);
...
#endif
#if (defined(RTCONFIG_PPTPD) || defined(RTCONFIG_ACCEL_PPTPD)) && !defined(RTCONFIG_MULTILAN_CFG)
		if (is_routing_enabled())
...
#if defined(RTCONFIG_PORT_BASED_VLAN) && !defined(RTCONFIG_MULTILAN_CFG)
	/* Write dhcpd config for vlan's subnet */
	vlan_subnet_dnsmasq_conf(fp);
#endif
#if defined(RTCONFIG_TAGGED_BASED_VLAN) && !defined(RTCONFIG_MULTILAN_CFG)
	/* Write dhcpd config for vlan's subnet */
	vlan_subnet_dnsmasq_conf(fp);
#endif
...
#if defined(RTAX55) || defined(RTAX1800) || defined(RTAX58U_V2) || defined(RTAX3000N)
...
#endif
#if defined(RTAX56_XD4) || defined(XC5)
	if((nvram_match("HwId", "A") && nvram_get_int("BLE_BT") == 99) ||
		(nvram_match("HwId", "C") && nvram_get_int("BLE_BT") == 99)){
		/* Master without BT */
		return;
...
#if !defined(BLUECAVE) && !defined(RTAX95Q) && !defined(XT8PRO) && !defined(BM68) && !defined(XT8_V2) && !defined(RTAXE95Q) && !defined(ET8PRO) && !defined(ET8_V2) && !defined(RTAX56_XD4) && !defined(XD4PRO) && !defined(RTAX82_XD6) && !defined(RTAX82_XD6S) && !defined(ET12) && !defined(XT12) && !defined(XC5)
...
setup_nc_event_conf()
{
	...
	if (!strcmp(nvram_safe_get("nc_setting_conf"), "") ||
		...
		for (i=0; mapInfo[i].value != 0; i++) {
			...
			snprintf(tmp, sizeof(tmp), "<%x>%d>%d", mapInfo[i].value, action, mapInfo[i].eType);
...
#if defined(RTAX95Q) || defined(XT8PRO) || defined(BM68) || defined(XT8_V2) || defined(RTAXE95Q) || defined(ET8PRO) || defined(ET8_V2) || defined(RTAX56U) || defined(RTAX56_XD4) || defined(XD4PRO) || defined(CTAX56_XD4) || defined(ET12) || defined(XT12) || defined(XC5)
	return 1;
#endif
...
//#if defined(RTCONFIG_JFFS2LOG) && defined(RTCONFIG_JFFS2)
	...
	else if(strcmp(script, "resetdefault") == 0)
	{
		...
		// if ate needs, TBD. except writing flash
		_dprintf("nothing to do in rc service(ate), skip it\n");
		goto skip;
#endif
		...
		_dprintf("<< wait upgrading %d secs\n", i);
...
retry_wps_enr:
 ...
 && !defined(RTCONFIG_CONCURRENTREPEATER)
	...
	else if (strcmp(script, "wpasupp_qis")==0)	// rp qis dedicated preauth
	{
		...
		_dprintf("%s wpasupp_qis.. wl%d, cli:%d(%d/%d), reload_wait:%d\n", __func__, band, client_mode, dpsta_mode(), dpsr_mode(), reload_wait);
		...
		if(action & RC_SERVICE_START)
		{
			if(client_mode) {
				...
				if(band >= 0) {
					...
					{
						if(nvram_match("x_Setting", "1")) {
							_dprintf("backup current wlc settings\n");
...
#if defined(RTCONFIG_MTK_BSD)
#if (defined(RTCONFIG_WLMODULE_MT7915D_AP) && defined(XD4S)) || defined(RTCONFIG_MT798X)
void stop_mtk_bs20(void)
{
	if(pids("bs20"))
	{
