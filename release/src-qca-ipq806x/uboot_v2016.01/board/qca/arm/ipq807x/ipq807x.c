...
extern unsigned smem_read_alloc_entry(smem_mem_type_t type, void *buf, int len);
extern void gpio_direction_output(unsigned int gpio, unsigned int out);
...
int get_napa_gpio(u32 napa_gpio[2])
{
...
void board_pci_deinit(void)
{
	...
	for (i = 0; i < PCI_MAX_DEVICES; i++) {
		...
		err = fdt_get_named_resource(gd->fdt_blob, node, "reg", "reg-names", "parf",
				&parf);
		if (err < 0)
			printf("%s: get parf node failed. (err %d)\n", __func__, err);
		...
		if (err < 0) {
			...
			if(soc_ver_major == 1) {
				err = fdt_get_named_resource(gd->fdt_blob, node, "reg", "reg-names", "pci_phy_gen2",
					     &pci_phy);
				if (err < 0) {
					printf("%s: get pci_phy_gen2 node failed. (err %d)\n", __func__, err);
					return;
				}
				...
				err = fdt_get_named_resource(gd->fdt_blob, node, "reg", "reg-names", "pci_phy_gen3",
					     &pci_phy);
				if (err < 0) {
					printf("%s: get pci_phy_gen3 node failed. (err %d)\n", __func__, err);
					return;
				}
			} else {
				printf("%s: get pci_phy node failed. (err %d)\n", __func__, err);
				return;
			}
		}
...
void fdt_fixup_cpr(void *blob)
{
	...
	int i, ret;
	uint64_t opp_hz[] = {1017600000LL, 1382400000LL, 1651200000LL,
			     1843200000LL, 1920000000LL, 2208000000LL};
